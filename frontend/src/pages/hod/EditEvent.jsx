  ...
  return (
    <div className="dashboard-content anim-fade-in">
      <div className="dash-header-flex">
        <h2 className="dash-title">Edit Event: {eventData.title}</h2>
        <button onClick={() => navigate('/hod')} className="btn-ghost">Back to List</button>
      </div>
      
      <form className="create-event-form card glass-card" onSubmit={handleSubmit}>
            ...
            {eventData.games.map((game, index) => (
              <div key={index} className="game-form-item card">
                ...
                <div className="form-group">
                  ...
                  <textarea value={game.rules} onChange={(e) => handleGameChange(index, 'rules', e.target.value)} placeholder="Rules, point system, etc." required />
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="form-section">
          ...
          <div className="form-row">
            ...
            <div className="form-group">
              <label>UPI ID</label>
              <input name="upiId" value={eventData.upiId} onChange={handleChange} required />
            </div>
          </div>
        </div>
